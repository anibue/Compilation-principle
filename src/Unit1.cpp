...
const  NORW  =  16;  /* # OF RESERVED WORDS */    
...
void STATEMENT(SYMSET FSYS,int LEV,int &TX) {   /*STATEMENT*/
  ...
  switch (SYM) {
	...
	case CALLSYM:
		...
		break;
  /*    ifsym          ifsym        ifsym      ifsym     ifsym    ifsym      */
	case IFSYM:
    /*
     * IF-THEN-ELSE 代码生成：
     * 1) 先生成条件跳转 JPC（假分支地址回填）
     * 2) 编译 THEN 语句
     * 3) 若存在 ELSE：生成 JMP 跳过 ELSE，再分别回填两处地址
     */
		GetSym();
...
void __fastcall TForm1::ButtonRunClick(TObject *Sender) {
  ...
  WSYM[13]=THENSYM;    WSYM[14]=VARSYM;
  WSYM[15]=WHILESYM;   WSYM[16]=WRITESYM;
       

  /* 初始化单字符运算符映射，
  将源程序中的单字符符号转换为编译器可识别的 Token 类型
  不再使用 # 作为不等号） */
  ...
  if (FIN!=0) {
    ...
    Form1->printfs("Cannot open PL/0 source file.");
  }
}
